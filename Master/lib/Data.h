...
class Data {
public:
  Data(uint8_t numOptions, const char *const *PROGMEM names,
  ...
  const char *getName(uint8_t width) {
    ...
    // Copy up to width bytes from PROGMEM, stop on NUL, preserve trailing
    // spaces
    if (src) {
  ...
  uint8_t value() { return this->index; }
  ...
  uint8_t length() { return this->numOptions; }
